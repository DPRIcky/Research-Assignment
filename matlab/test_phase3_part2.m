% test_phase3_part2.m - Test Phase 3 (Part 2): LQR Controller
% Tests Linear Quadratic Regulator trajectory tracking controller
...
fprintf('===== PHASE 3 TESTING: LQR CONTROLLER =====\n\n');

%% Test 1: LQR - Straight Line Tracking
fprintf('Test 1: LQR - Straight Line Tracking\n');
fprintf('--------------------------------------\n');
...
% State weights: [x, y, theta, v]
Q = diag([10, 10, 5, 1]);
R = diag([1, 10]); % Control weights: [a, delta]
controller = LQRController(Q, R, dt, 'bicycle', vehicle.L);
fprintf('Created LQR controller (Q_x=%.1f, Q_theta=%.1f, R_a=%.1f)\n', ...
        Q(1,1), Q(3,3), R(1,1));
...
% Plot results
figure('Name', 'Test 1: LQR Straight Line Tracking');
subplot(2,2,1);
...
%% Test 2: LQR - Circular Path Tracking
fprintf('Test 2: LQR - Circular Path Tracking\n');
fprintf('-------------------------------------\n');
...
figure('Name', 'Test 2: LQR Circular Path Tracking');
subplot(2,2,1);
...
%% Test 3: LQR - Following Planned Path
fprintf('Test 3: LQR - Following Planned Path\n');
fprintf('-------------------------------------\n');
...
if stats_plan.success
    ...
    figure('Name', 'Test 3: LQR Following Planned Path');
    env.plot();
    hold on;
    ...
    legend('Location', 'best');
    title('LQR Tracking Planned Path');
    axis equal; grid on;
else
...
%% Test 4: Weight Tuning Comparison
fprintf('Test 4: LQR Weight Tuning Comparison\n');
fprintf('-------------------------------------\n');

% Test different weight configurations
weight_sets = {
    struct('name', 'Low Q', 'Q', diag([1, 1, 1, 0.1]), 'R', diag([1, 10])),
    struct('name', 'Medium Q', 'Q', diag([10, 10, 5, 1]), 'R', diag([1, 10])),
    struct('name', 'High Q', 'Q', diag([50, 50, 20, 5]), 'R', diag([1, 10]))
};
...
fprintf('Testing weight sets on figure-8 path:\n\n');

for g = 1:length(weight_sets)
    weights = weight_sets{g};
    
    % Create controller with specific weights
    ctrl = LQRController(weights.Q, weights.R, dt, 'bicycle', vehicle.L);
    ...
    fprintf('%s: mean=%.3fm, max=%.3fm\n', weights.name, mean_err, max_err);
end

fprintf('\n');

%% Test 5: LQR vs PID Comparison
fprintf('Test 5: LQR vs PID Comparison\n');
fprintf('------------------------------\n');
...
% Test LQR
lqr_ctrl = LQRController(diag([10, 10, 5, 1]), diag([1, 10]), dt, 'bicycle', vehicle.L);
state_lqr = [0, 0, 0, 0];
T = 25;
steps = round(T / dt);
...
%% Summary
fprintf('=====================================\n');
fprintf('PHASE 3 (LQR CONTROLLER) TESTS COMPLETE!\n');
fprintf('=====================================\n\n');

fprintf('Summary:\n');
fprintf('  ✓ LQR controller implementation working\n');
fprintf('  ✓ Tracks straight and curved paths\n');
fprintf('  ✓ Follows planned paths from planners\n');
fprintf('  ✓ Tunable weights for performance\n');
fprintf('  ✓ Outperforms PID on complex paths\n\n');

fprintf('Next: Implement MPC controller (Phase 3 Part 3)\n\n');
